...
void DecodeData::Clusterize(int numnum, int Jinfnum, calib *cal)
{
	...
	for (int side = 0; side < 2; side++)
	{
		...
		int border[] = {0, 1, 2, 62, 63, 64, 65, 66, 126, 127, 128, 129, 130, 190, 191, 192, 193, 194, 254, 255, 256, 257, 258, 318, 319, 320, 321, 322, 382, 383, 384, 385, 386, 446, 447, 448, 449, 450, 510, 511, 512, 513, 514, 574, 575, 576, 577, 578, 638, 639, 640, 641, 642};
		...
		float ssun[arraysize];

		for (int count = 0; count < arraysize; count++)
		{
			int va = (int)(count / nchava);
			ssun[count] = (array[count] / 8.0 - pede[count] - CN[va]);
		}

		ssonmax = TMath::MaxElement(arraysize, ssun);
		seedaddmax = TMath::LocMax(arraysize, ssun);

		if (!(std::find(std::begin(border), std::end(border), seedaddmax) != std::end(border)))
		{
			if (ssonmax > highthreshold)
			{
				seedfound = true;
				clusadd = seedaddmax;
				cluslen = 1;
