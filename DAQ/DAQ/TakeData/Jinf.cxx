...
//--------------------------------------------------------------
Jinf::Jinf(char* name,char* conf_file,int address,AMSWcom* node_in,int flagfake):JinjSlave( name, conf_file, address, node_in, flagfake){
  //  logfile=log;
  FlagFake=flagfake;
  sprintf(myname,"%s",name);
  if(FlagFake==2) PRINTF("Creating a fake Jinf!\n");
  selfaddress=address;
