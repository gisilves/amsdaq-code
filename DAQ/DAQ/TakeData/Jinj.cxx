...
Jinj::Jinj(const char* name, const char* conf_file, int address, AMSWcom* node_in, int flagfake){ // gcc 4.3 : added 'const'
char namejinf[50];
FILE *stream;
FlagFake=flagfake;
...
for (int ii=0;ii<NSLAVE;ii++) {
  ...
  if((CPars->refmask&(1<<ii)) && strstr(CPars->SlaveConfFile[ii],"JINF")) {
    if(FlagFake && nJinfs>0) {
      ...
      if(!Slave[NSlave]) {
        Slave[NSlave]= new Jinf(namejinf,CPars->SlaveConfFile[ii],addr,node,flagfake);
        SlaveAdd[NSlave]=ii;
        NSlave++;
        nJinfs++;
      }
    ...
    if (!FlagFake){//if fake Jinj cannot be any JLV1...
      ...
      int addr=ii<<8|0x3f;
      if(!Slave[NSlave]) {
        Slave[NSlave]= new JLV1(namejinf,CPars->SlaveConfFile[ii],addr,node,flagfake);
        SlaveAdd[NSlave]=ii;
        NSlave++;
      }
    }
  ...
  /* Dampe remote controller - by Andrea Nardinocchi */
    ...
    if (FlagFake) {
      ...
      int addr = ii<<8|0x3f;
      if (!Slave[NSlave]) {
        Slave[NSlave] = new c_dampe(namejinf, CPars->SlaveConfFile[ii], addr, node, flagfake);
        SlaveAdd[NSlave] = ii;
        NSlave++;
      }
    }
